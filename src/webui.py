...
def generate_from_prompt(lrc_file, prompt, audio_length, output_dir):
    ...
    try:
        ...
        if os.path.exists(audio_file) and os.path.getsize(audio_file) > 0:
            ...
            print(f"Audio file generated at: {os.path.abspath(audio_file)}")
            print(f"Audio file size: {os.path.getsize(audio_file)} bytes")
            # Convert WAV to MP3 (force overwrite with -y)
            mp3_file = os.path.join(output_dir, "output.mp3")
            subprocess.run(f"ffmpeg -y -i {audio_file} {mp3_file}", shell=True, check=True)
            # Return absolute path for Gradio compatibility
            return f"Song generated successfully in {output_dir}", os.path.abspath(mp3_file)
        else:
...
def generate_from_audio(lrc_file, audio_file, audio_length, output_dir):
    ...
    try:
        ...
        if os.path.exists(audio_file_path) and os.path.getsize(audio_file_path) > 0:
            return f"Song generated successfully in {output_dir}", os.path.abspath(audio_file_path)
        else:
            return f"Error: Generated audio file is empty or missing", None
    except Exception as e:
        return f"Error: {str(e)}", None
...
if __name__ == "__main__":
    with gr.Blocks(theme="soft", title="DiffRhythm V1.2 WebUI") as demo:
        ...
        with gr.Tab("Generate from Prompt"):
            ...
            audio_preview_prompt = gr.Audio(label="Preview and Download Generated Song", type="filepath", interactive=True, visible=True, format="mp3")
        ...
        with gr.Tab("Generate from Audio"):
            ...
            audio_preview_audio = gr.Audio(label="Preview and Download Generated Song", type="filepath", interactive=True, visible=True, format="mp3")
