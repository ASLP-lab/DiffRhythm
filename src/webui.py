...
def generate_from_prompt(lrc_file, prompt, audio_length, output_dir):
    ...
    try:
        ...
        if os.path.exists(audio_file) and os.path.getsize(audio_file) > 0:
            ...
            # Return WAV file path directly (no conversion needed)
            return f"Song generated successfully in {output_dir}", audio_file
        else:
...
def generate_from_audio(lrc_file, audio_file, audio_length, output_dir):
    ...
    try:
        ...
        if os.path.exists(audio_file_path) and os.path.getsize(audio_file_path) > 0:
            return f"Song generated successfully in {output_dir}", audio_file_path
        else:
            return f"Error: Generated audio file is empty or missing", None
    except Exception as e:
        return f"Error: {str(e)}", None
...
if __name__ == "__main__":
    with gr.Blocks(theme="soft", title="DiffRhythm V1.2 WebUI") as demo:
        ...
        with gr.Tab("Generate from Prompt"):
            ...
            audio_preview_prompt = gr.Audio(label="Preview and Download Generated Song", interactive=False, show_download_button=True, show_share_button=False)
        ...
        with gr.Tab("Generate from Audio"):
            ...
            audio_preview_audio = gr.Audio(label="Preview and Download Generated Song", interactive=False, show_download_button=True, show_share_button=False)
